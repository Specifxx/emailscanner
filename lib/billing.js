...
export function explainStripeError(err) {
  ...
  // The portal refuses to open until its settings have been saved once, and
  // says so in a way that reads like a bug in the app.
  if (/configuration/i.test(message) && /portal|customer portal/i.test(message)) {
    return 'The Stripe customer portal has not been set up yet. Save its settings once in the Stripe dashboard under Settings → Billing → Customer portal, with "Cancel subscription" enabled.'
  }
