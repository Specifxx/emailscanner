...
export function buildAuthUrl(state) {
  const params = new URLSearchParams({
    ...
    response_mode: 'query',
    scope: SCOPES.join(' '),
    // Without this, "Add Outlook" silently re-links whichever account is
    // already signed in to Microsoft, so a second mailbox can never be added.
...
export function buildQuery(parsed, { withDate = true } = {}) {
  const clean = (term) => term.replace(/["\\]/g, '').trim()

  const clauses = []
  let length = 0
  for (const { term } of parsed.terms.slice(0, MAX_SEARCH_TERMS)) {
    const cleaned = clean(term)
    if (!cleaned) continue
    // Every term is quoted, phrase or not. Bare terms let KQL reinterpret
    // punctuation as syntax — a leading "-" means NOT, which silently inverts
    // the search for anyone who types a hyphenated word.
...
/**
 * Graph has no way to exclude folders from a $search, so junk and deleted mail
 * are filtered out by id afterwards. A 404 just means the mailbox has no such
 * folder; anything else is worth knowing about, since failing to resolve an id
 * silently lets that folder into the results.
 */
  ...
  const results = await Promise.all(
    names.map((name) =>
      graphGet(`${GRAPH}/me/mailFolders/${name}?$select=id`, accessToken)
        ...
        .catch((err) => {
          if (err.status !== 404) {
            console.error(`Could not resolve ${name} folder:`, err.message)
          }
          return null
        })
    )
 ...
 */
export async function search(accessToken, parsed, { limit = 100 } = {}) {
  const top = Math.min(limit, PAGE_SIZE)

  async function runSearch() {
    const kql = buildQuery(parsed)
    try {
      if (!kql) throw Object.assign(new Error('empty query'), { status: 400 })
      return await graphGet(searchUrl(kql, top), accessToken)
    } catch (err) {
      if (err.status !== 400) throw err
      // KQL date syntax is the least-documented part of $search, so drop the
      // time window and try again before giving up on keyword search. Ask for
      // a full page here: without the date clause the results span all time,
      // and the client-side window filter below would otherwise discard most
      // of a small page and return nothing.
      try {
        const undated = buildQuery(parsed, { withDate: false })
        if (!undated) throw Object.assign(new Error('empty query'), { status: 400 })
        return await graphGet(searchUrl(undated, PAGE_SIZE), accessToken)
      } catch (retryErr) {
