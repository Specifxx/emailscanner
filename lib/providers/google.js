...
export function buildAuthUrl(state) {
  const params = new URLSearchParams({
    ...
    // select_account is what makes "Add Google" able to attach a SECOND
    // mailbox — without it Google silently reuses whoever is already signed
    // in. consent is also needed because Google only returns a refresh token
    // on first consent unless forced.
...
/**
 * Deep link into the right mailbox. `/u/0` would open whichever Google account
 * is first in the browser, which is the wrong one as soon as two Gmail
 * mailboxes are connected; authuser pins it to this account.
 */
function threadUrl(threadId, accountEmail) {
...
async function fetchMetadata(accessToken, id, accountEmail) {
  ...
  return {
    ...
    url: threadUrl(message.threadId || message.id, accountEmail),
  }
}
 ...
 */
export async function search(accessToken, parsed, { limit = 100, accountEmail = '' } = {}) {
  const params = new URLSearchParams({
    q: buildQuery(parsed),
  ...
  async function worker() {
    while (cursor < ids.length) {
      const index = cursor++
      try {
        results[index] = await fetchMetadata(accessToken, ids[index].id, accountEmail)
      } catch (err) {
