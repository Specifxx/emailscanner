...
/** Lifetime scan count for the landing page. 0 if the row is missing. */
export async function totalScans() {
  const { data, error } = await db()
    .from('stats')
    .select('total_scans')
    .maybeSingle()

  if (error) fail('load the scan count', error)
  return Number(data?.total_scans || 0)
}
...
/**
 * Deletes the person entirely. `accounts` rows cascade via the foreign key in
 * supabase/schema.sql, so every connected mailbox's tokens go with it.
 */
